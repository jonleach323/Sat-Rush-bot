...
import { AFFILIATE_RATE_BPS, BUYBACKS_TO_STAKING_BPS, EPOCH_DEDUP_UPLIFT, RUSH_MINT_PER_USD, RUSH_MINT_USD_CAP, SATS_VAULT_CARRY_DAILY, STAKING_YIELD_DAILY, STREAK_GRACE_ROUNDS, STRIKE_BOOST_WINDOW_ROUNDS, STRIKE_HASHRATE_MULTIPLIER, STRIKE_PAYOUT_FRACTION, STRIKE_TRIGGER_MODULUS, TOKEN_VAULT_CARRY_DAILY, V2_DEPLOY_FEE_LAYER_BPS, V2_LOSING_TILE_REFUND_BPS, V2_VAULT_EXIT_FEE_BPS, VAULT_HASHRATE_PER_TICKET } from "../src/strategy/facts.js";
...
console.log(`  → the 1-BTC vault pays ~${(obUsd / projTickets / ticketNewFee).toFixed(1)}× (vs live-fee epoch) / ~${(obUsd / projTickets / (ticketLivePace ?? ticketOld)).toFixed(1)}× (vs 16 pace) the epoch vault per unit of hashrate; the vault engine ranks by marginal ticket value each tick.`);
